        ...
        <form action="{{route('courses.update', $course->id)}}" method="post" enctype="multipart/form-data" class="col-6">
            ...
            <div class="main mb-5">
                ...
                <div class="">
                    ...
                    <img style="width: 7rem;" src="{{\Illuminate\Support\Facades\Storage::disk('local')->url($course->image)}}" />
            ...
            <div class="details">
                <div class="form-group">
                    ...
                    <input type="text" name="header_desc" value="{{$course->details->header_desc}}" placeholder="header_desc" id="header_desc" class="form-control"/>
                </div>

                <div class="form-group">
                    <label for="description" >description :</label>
                    <input type="text" name="description" value="{{$course->details->description}}" placeholder="description" id="description" class="form-control"/>
                </div>

                <div class="form-group contents">
                    <button type="button" onclick="addContent(event)" class="add">Add Input</button>
                    @foreach($course->details->content as $content)
                ...
                <div class="form-group contents">
                    ...
                    @for($i=0;$i<count($course->details->includes->titles);$i++)
                        <div class="row">
                            <div class="col">
                                <input type="text" name="includes_titles[]" value="{{$course->details->includes->titles[$i]}}" placeholder="title" id="title" class="form-control"/>
                            </div>
                            <div class="col">
                                <input type="text" name="includes_icons[]" value="{{$course->details->includes->icons[$i]}}" placeholder="icon" id="icon" class="form-control"/>
                            </div>
                        </div>
                    @endfor
                </div>
